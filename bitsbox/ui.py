...
@blueprint.route('/drawer/<int:drawer_id>/collections/new', methods=['GET', 'POST'])
def drawer_collection_new(drawer_id):
    db = get_db()
    ...
    if request.method == 'GET':
        return render_template('collection_add.html', **context)
    ...
    contents_count = int(request.values.get('contents_count', 0))

    # Create the collection
    db.execute('''
        INSERT INTO
            collections (name, description, contents_count, drawer_id)
        VALUES
            (?, ?, ?, ?)
    ''', (name, description, contents_count, drawer_id))
    db.commit()

    return redirect(url_for('ui.drawer', drawer_id=drawer_id))
